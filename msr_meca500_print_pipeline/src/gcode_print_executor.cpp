...
#include "gcode.hpp"
#include "msr_meca500_print_pipeline/srv/goal.hpp"
#include <fstream>
#include "msr_meca500_print_pipeline/srv/gcode_file.hpp"
#include <fcntl.h>
#include <termios.h>
#include <unistd.h>
#include <cstdio>
...
class GcodePrintExecutor : public rclcpp::Node
{
public:
  GcodePrintExecutor()
  : Node("gcode_print_executor",
  ...
  void init() {
    ...
    // -- machine_settings.yaml / print_tuning.yaml (loaded by <param from> in the launch file) --------
    cfg_ee_link_with_extruder    = param_or<std::string>("ee_link_with_extruder", cfg_ee_link_with_extruder);
    cfg_ee_link_without_extruder = param_or<std::string>("ee_link_without_extruder", cfg_ee_link_without_extruder);
    cfg_tip_offset_with_extruder    = param_or<double>("tip_offset_with_extruder_m", cfg_tip_offset_with_extruder);
    cfg_tip_offset_without_extruder = param_or<double>("tip_offset_without_extruder_m", cfg_tip_offset_without_extruder);
    {
      auto home_xy = param_or<std::vector<double>>("home_xy_mm", {cfg_home_x, cfg_home_y});
      if (home_xy.size() >= 2) { cfg_home_x = home_xy[0]; cfg_home_y = home_xy[1]; }
    }
    cfg_home_feedrate           = param_or<int>("home_feedrate", cfg_home_feedrate);
    cfg_e_steps_per_mm          = param_or<double>("e_steps_per_mm", cfg_e_steps_per_mm);
    cfg_physical_min_e_steps    = param_or<double>("physical_min_e_steps", cfg_physical_min_e_steps);
    cfg_min_feed_rate           = param_or<double>("min_feed_rate", cfg_min_feed_rate);
    cfg_max_feed_rate           = param_or<double>("max_feed_rate", cfg_max_feed_rate);
    cfg_transition_retract_mm   = param_or<double>("transition_retract_mm", cfg_transition_retract_mm);
    cfg_rehome_chance_per_thousand = param_or<int>("rehome_chance_per_thousand", cfg_rehome_chance_per_thousand);
    cfg_rehome_skip_last_layers    = param_or<int>("rehome_skip_last_layers", cfg_rehome_skip_last_layers);
    ...
    goal_server = this->create_service<msr_meca500_print_pipeline::srv::Goal>(
      "goal_service",
      std::bind(&GcodePrintExecutor::goal_callback, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::ServicesQoS(),
      service_cb_group);

    gcode_file_server = this->create_service<msr_meca500_print_pipeline::srv::GcodeFile>(
      "gcode_file_service",
      std::bind(&GcodePrintExecutor::gcode_file_callback, this,
                std::placeholders::_1, std::placeholders::_2),
      rclcpp::ServicesQoS(),
      service_cb_group);
    ...
    // End effector link + contact-point offset (machine_settings.yaml / print_tuning.yaml)
    if (use_extruder) {
      threshold = cfg_tip_offset_with_extruder;
      end_effector_link = cfg_ee_link_with_extruder;
    }
    else {
      threshold = cfg_tip_offset_without_extruder;
      end_effector_link = cfg_ee_link_without_extruder;
    }
  }
...
private:
  ...
  rclcpp::Service<msr_meca500_print_pipeline::srv::Goal>::SharedPtr goal_server;
  rclcpp::Service<msr_meca500_print_pipeline::srv::GcodeFile>::SharedPtr gcode_file_server;
  ...
  // machine_settings.yaml / print_tuning.yaml, read in init() via <param from>. Defaults here are the
  // fallback when the node is run without the config.
  std::string cfg_ee_link_with_extruder = "nozzle";
  std::string cfg_ee_link_without_extruder = "link_6__flange";
  double cfg_tip_offset_with_extruder = 0.008;
  double cfg_tip_offset_without_extruder = 0.0025;
  double cfg_home_x = -3.00;
  double cfg_home_y = -10.00;
  int cfg_home_feedrate = 1200;
  double cfg_e_steps_per_mm = 93.0;
  double cfg_physical_min_e_steps = 30.0;
  double cfg_min_feed_rate = 17.5;
  double cfg_max_feed_rate = 165.0;
  double cfg_transition_retract_mm = 1.0;
  int cfg_rehome_chance_per_thousand = 5;
  int cfg_rehome_skip_last_layers = 3;

  template <typename T>
  T param_or(const std::string& name, const T& fallback) {
    return this->has_parameter(name) ? this->get_parameter(name).get_value<T>() : fallback;
  }
  ...
  void home_bed(bool e_relative_mode) {
    if (use_extruder && ender_ready) {
      ...
      char cmd[64];
      std::snprintf(cmd, sizeof(cmd), "G1 X%.2f Y%.2f F%d",
                    cfg_home_x, cfg_home_y, cfg_home_feedrate);
      send_ender(cmd);
    }
  }
  ...
  void gcode_file_callback(
      const std::shared_ptr<msr_meca500_print_pipeline::srv::GcodeFile::Request>  req,
            std::shared_ptr<msr_meca500_print_pipeline::srv::GcodeFile::Response> res)
  {
    // Read file
    ...
    auto goal_req = std::make_shared<msr_meca500_print_pipeline::srv::Goal::Request>();
    auto goal_res = std::make_shared<msr_meca500_print_pipeline::srv::Goal::Response>();
  ...
  void goal_callback(
    const std::shared_ptr<msr_meca500_print_pipeline::srv::Goal::Request>  request,
          std::shared_ptr<msr_meca500_print_pipeline::srv::Goal::Response> response) {
...

    {
      ...
      for (auto& tb : trajectory_batches) {
        ...
        if (tb.type != MoveType::None) {
          ...
          // Deliberately not flooring e_sum here: raw_matched_f below should
      ...
      // Corner-taper floor: the smallest E move a print segment may command.
      // A single G1 E below ~30 stepper steps doesn't reliably move the
      // extruder at all -- tested directly on mini_cube: 20 steps still
      // dropped a corner, 30 cleared it. Anything nonzero-but-tinier gets
      // bumped up to this. Cost is some local over-extrusion; a missing
      // corner is worse. (A per-print percentile of this print's own e_sum
      // distribution was tried here instead but never came out above ~8
      // steps on any real print, so it never did anything -- removed.)
      const double kEStepsPerMm = cfg_e_steps_per_mm;              // machine_settings.yaml / print_tuning.yaml (M503)
      const double kPhysicalMinESteps = cfg_physical_min_e_steps;  // machine_settings.yaml / print_tuning.yaml
      dynamic_min_print_e_sum = kPhysicalMinESteps / kEStepsPerMm;
      RCLCPP_INFO(this->get_logger(), "Corner-taper floor: %.0f steps -> %.4fmm",
        kPhysicalMinESteps, dynamic_min_print_e_sum);
    }
    ...
    if (moved_to_bed) {
      ...
      auto execute_batch = [this](moveit::planning_interface::MoveGroupInterface::Plan& plan,
        ...
        if (use_extruder && ender_ready && type != MoveType::None) {
          ...
          if (type == MoveType::Retract) {
            ...
            const double kMinFeedRate = cfg_min_feed_rate;   // mm/min, machine_settings.yaml / print_tuning.yaml -- soft floor
            const double kMaxFeedRate = cfg_max_feed_rate;   // mm/min, machine_settings.yaml / print_tuning.yaml -- hotend ceiling
      ...
      const int kLastFewLayers = cfg_rehome_skip_last_layers;  // machine_settings.yaml / print_tuning.yaml
      ...
      const int kRehomeChancePerThousand = cfg_rehome_chance_per_thousand;  // machine_settings.yaml / print_tuning.yaml
        ...
        if (b + 1 < trajectory_batches.size()) {
          ...
          if (!do_rehome) {
            ...
            if (use_extruder && ender_ready) {
              ...
              {
                char rcmd[64];
                std::snprintf(rcmd, sizeof(rcmd), "G1 E%.3f F%d",
                              -cfg_transition_retract_mm, cfg_home_feedrate);
                send_ender(rcmd);
              }
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<GcodePrintExecutor>();
