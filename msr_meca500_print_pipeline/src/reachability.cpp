...
public:
  ...
  {    
    // All from machine_settings.yaml / print_tuning.yaml (<param from> in the launch files).
    ...
    this->declare_parameter("ee_link_with_extruder", "nozzle");
    this->declare_parameter("ee_link_without_extruder", "link_6__flange");
    this->declare_parameter("tip_offset_with_extruder_m", 0.008);
    this->declare_parameter("tip_offset_without_extruder_m", 0.0025);
    this->declare_parameter("approach_sweep_m", std::vector<double>{0.05, 0.35, 0.02});
  }
...
private:
  ...
  void run_sweep()
  {
    ...
    // +3mm over the print tip offset so the arm hovers clear of the bed
    // during the sweep -- it isn't extruding, so a bump is pure downside.
    const double kSweepBedClearance = 0.003;
    if (use_extruder_) {
      end_effector_link_ = this->get_parameter("ee_link_with_extruder").as_string();
      threshold = this->get_parameter("tip_offset_with_extruder_m").as_double() + kSweepBedClearance;
    }
    else {
      end_effector_link_ = this->get_parameter("ee_link_without_extruder").as_string();
      threshold = this->get_parameter("tip_offset_without_extruder_m").as_double();
    }
    ...
    const auto sweep = this->get_parameter("approach_sweep_m").as_double_array();
    const double sweep_start = sweep.size() > 0 ? sweep[0] : 0.05;
    const double sweep_stop  = sweep.size() > 1 ? sweep[1] : 0.35;
    const double sweep_step  = sweep.size() > 2 ? sweep[2] : 0.02;
    ...
    for (double t = sweep_start; t <= sweep_stop; t += sweep_step) {
